...
function findMyJob() {
  ...
  for (const job of jobs) {
    ...
    if (jobLocation.includes(location) && jobTitle.includes(jobDescription)) {
...
function displayResults(results) {
  ...
  if (results.count === 0) {
    ...
    cell.textContent = 'Nessun lavoro trovato.';
    row.appendChild(cell);
    tbody.appendChild(row);
    return;
  }

  for (const job of results.result) {
    const row = document.createElement('tr');
    const locationCell = document.createElement('td');
    locationCell.textContent = job.location;
    const descriptionCell = document.createElement('td');
    descriptionCell.textContent = job.title;
...
const jobs = [
  ...
  { title: "Hands-On QA Leader", location: "IL, Tel Aviv, Israel" },
  { title: "Southend-on-Sea Traineeships", location: "GB, SOS, Southend-on-Sea" },
  { title: "Visual Designer", location: "US, NY, New York" },
